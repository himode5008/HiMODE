...
import sys
import files
from files.CubePad import CustomPad
...
def e2c(equirectangular):
    cube = files.Equirec2Cube.ToCubeTensor(equirectangular.cuda())
    return cube

def c2e(cube):
    equirectangular = files.Cube2Equirec.ToEquirecTensor(cube.cuda())
...
class CETransform(nn.Module):
    def __init__(self):
        ...
        for h in equ_h:
            a = files.Equirec2Cube(1, h, h*2, h//2, 90)
            self.e2c['(%d,%d)' % (h, h*2)] = a

        for h in cube_h:
            a = files.Cube2Equirec(1, h, h*2, h*4)
            self.c2e['(%d)' % (h)] = a
...
class MyModel(nn.Module):
    def __init__(self, layers, decoder, output_size=None, in_channels=3, pretrained=True):
        ...
        mypad = getattr(files.CubePad, 'SpherePad')
        ...
        self.grid = files.Equirec2Cube(None, 512, 1024, 256, 90).GetGrid()
        self.d2p = files.Depth2Points(self.grid)
