...
def Run(loader, model, crop):
    ...
    pbar.set_description('Process of validation')
    gpu_num = torch.cuda.device_count()
    os.system('mkdir -r Result')

    CE = files.CETransform()
    count = 0
    ...
    with torch.no_grad():
        for it, data in enumerate(pbar):
            ...
            # Converting to Numpy and Normalize to 0 and 1
            ...
            for i in range(dep_np.shape[0]):
                ...
                # Cropping area is 68 to up and down 
                ...
                imwrite('Result/Both%.3d.jpg'%count, (big*255).astype(np.uint8))
                imwrite('Result/Depth%.3d.jpg'%count, (only_dep*255).astype(np.uint8))
                d = {
                            'RGB': cat_rgb,
                            'depth': d_tmp[i, 0, ...]
                        }
                np.save('Result/Lout%.3d.npy'%count, d)
...
def main():
    ...
    saver = files.ModelSaver('./model')
    from modules.tranED import MyModel as HardNet
    model = HardNet(
    		layers=50,
    		decoder="upproj",
    		output_size=None,
